...
const ConfimationDialog = ({ open, deleteHandler, handleClose }: IConfimationDialog) => {
    return(
        <Dialog
        open={open}
        onClose={handleClose}
      ...
      >
        ...
        <DialogActions>
          ...
          <Button onClick={deleteHandler} autoFocus>Confirm</Button>
        </DialogActions>
      </Dialog>
    )
}
...
const MovieForm = ({ className, header, title, year, id, url }: IProps) => {
    ...
    const display = (isDragActive: boolean) => {
        ...
        if(moviePoster) {
            return <img 
                height="200"
                src={typeof moviePoster === "string" ? moviePoster : URL.createObjectURL(moviePoster)} />
        } else {
            if(isDragActive) {
    ...
    const {getRootProps, getInputProps, isDragActive} = useDropzone({onDrop, disabled: !!id})

    const onChangeText = (label: Label) => ({target}: ChangeEvent<HTMLInputElement>) => {
        switch(label) {
            case Label.title:
    ...
    const validate = async () => {
        const {error} = movieSchema.validate({ title: movieTitle, year:movieYear});
        ...
        if(error) {
            ...
            setError({title: null, year: null, poster: null, [context?.label ?? "title"]: message});
        } else if(!moviePoster) {
  ...
  return (
    <Container className={className}>
        ...
        <Box className="detail-form-cont">
            ...
            <FormControl>
                <Box className="file-upload-cont file-upload-cont-mobile"  {...getRootProps()}>
                    <input {...getInputProps()} className="file-input"/>
                    {display(isDragActive)}
                </Box>
            </FormControl>
            ...
            <FormControl className="detail-button-cont">
                {isLoading ? <CircularProgress/> : <>
                    ...
                    {id && <Button onClick={() => { setDialog(true) }} color="error">Delete</Button>}
                    <Button onClick={validate}>Submit</Button> 
                </>}
            </FormControl>
        </Box>

        <ConfimationDialog open={isOpenDialog} deleteHandler={deleteHandler} handleClose={handleClose}/>
    </Container>
  );
}
