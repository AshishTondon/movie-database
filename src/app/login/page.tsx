...
import { Button, Container, TextField, Typography, styled, Checkbox, FormControl, FormControlLabel } from "@mui/material";
...
const Login = ({ className }: IWithClassName) => {
    ...
    const onChangeText = (label: string) => ({target}: ChangeEvent<HTMLInputElement>) => {
        switch(label) {
            case Label.email:
    ...
    const validate = () => {
        ...
        if(error) {
            ...
            setError(() => ({email: null, password: null, [context?.label ?? Label.email]: message}))
  ...
  return (
    <Container className={className}>
        ...
        <FormControl>
            <TextField placeholder="Email" onChange={onChangeText(Label.email)} helperText={error.email} error={!!error.email}/>
        </FormControl>
        <FormControl>
            <TextField placeholder="Password" type="password" onChange={onChangeText(Label.password)} error={!!error.password} helperText={error.password}/>
        </FormControl>
