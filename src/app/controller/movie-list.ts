...
const getImageList = async (page?: string | null) => {
  try {
    ...
    }).then(({resources, next_cursor} : {resources: Array<CloudinaryResource>, next_cursor: string}) => 
      ({list: resources.map(({public_id, context, secure_url}) => 
        ({url: secure_url, title: context?.custom?.title, year: context?.custom?.year, id: public_id})), nextPage: next_cursor}));

    return NextResponse.json(list, { status: 200 });
  } catch(err) {
    console.error("Error at delete movie", err);

    return NextResponse.json({message: "Internal server error.", list: []}, { status: 500 });
  }
}
