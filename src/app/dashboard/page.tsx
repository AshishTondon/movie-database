...
import { Button, Container, Typography, Box, styled, IconButton } from "@mui/material";
...
const DashboardHeader = ({ className }: IWithClassName) => {
    ...
    <Box className={className}>
        <Box component="span" className="title-cont">
            ...
            <IconButton href="/movie/create"><AddCircleOutlineIcon /></IconButton>
        </Box>
        <IconButton onClick={handleSignOut}><LogoutIcon /></IconButton>  
    </Box>
)};

const Dashboard = ({ className }: IWithClassName) => {
    ...
    const loadMore = () => {
        ...
        request<IMovie>({
            ...
            if(movies) {
                setMovies((movieArr) =>  
                    [...movieArr ?? [], ...movies.list].filter((value, index, self) =>
  ...
  return (
        <Container className={className}>
            {(!isLoading && movies && movies.length === 0) ? <EmptyContainer /> : 
            <>
                <DashboardHeader className="dashbaord-header" />
                    <Box className="tab-cont">
                        {movies?.map(({id, title, url, year}) => <Tab key={id} title={title} url={url} year={year} id={id}/>)}
                    </Box>
                    <Box className="tab-cont paging-cont">
                        {isLoading && <CircularProgress className="process-bar"/>}
                        {(!isLoading && nextPageId) && <Button className="process-bar" variant="outlined" onClick={loadMore}>Load More</Button>}
                    </Box>
            </>}
